...
const LookupStipendSchema = z.object({
    ...
    schedule: z.enum(["4x10", "5x8", "5x10", "6x10", "7x12", "custom"]).optional().default("5x10"),
    ...
    insurance_plan: z.enum(["none", "single", "family", "aca", "private", "union"]).optional().default("none"),
...
export async function POST(req: Request) {
    try {
        ...
        const deltaTo90 = round2(targetStipend - financials.breakdown.stipend_weekly);
        ...
        const netAfterInsuranceWeekly = round2(financials.breakdown.net_weekly - insuranceMid);

        if (d.ingest) void ingestReport(d, location, gsa, financials, insurance, insuranceMid, ip);
...
async function handleConstruction(d: any, location: any, gsa: any, hud: any, rent1Br: number, fiscalYear: number, ip: string) {
    ...
    const pctOfGsa = gsaComparison > 0 ? clamp(round2((pay.weekly_per_diem / gsaComparison) * 100), 0, 200) : 0;
    ...
    const netWeekly = round2(pay.weekly_wage - taxEstWeekly + pay.weekly_per_diem);
        ...
        data: {
            ...
            metadata: {
                ...
                offer_verdict: {
                    ...
                    gsa_comparison_basis: housingModel === "company" ? "M&IE only (company provides lodging)" : "Full GSA (lodging + M&IE)",
                },
                assumptions: [
                    `Trade: ${trade} · Schedule: ${schedule} · ${pay.total_hours}hr/wk`,
                    `Straight: ${pay.straight_hours}hr × $${hourlyRate}/hr = $${pay.straight_pay}`,
                    pay.ot_hours > 0 ? `OT: ${pay.ot_hours}hr × $${round2(hourlyRate * 1.5)}/hr = $${pay.ot_pay}` : null,
...
// ━━━ HEALTHCARE INGEST ━━━
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function ingestReport(d: any, location: any, gsa: any, financials: any, insurance: any, insuranceMid: number, ip: string) {
    try {
        const serviceClient = createServiceClient();
        await serviceClient.from("pay_reports").insert({
            zip: d.zip,
