...
interface ConstructionResult {
    ...
    housing: { hud_fmr_1br: number; stipend_monthly_est: number; stipend_surplus_monthly: number };
    negotiation: { pct_of_max: number };
    insurance: { plan: string; weekly_mid: number | null; source_type: string; weekly_min: number | null; weekly_max: number | null };
    derived: { net_after_insurance_weekly: number; gsa_comparison_weekly: number };
    metadata: {
...
const AGENCY_CHIPS = [
    "Aerotek", "CCS", "GRUS", "Kodiak", "Labor Finders",
    "PeopleReady", "Skillit", "TradeWorX", "Tradesmen Int'l", "Trillium",
];
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <span style={{ fontFamily: f.sans, fontSize: "16px", fontWeight: 700, color: T.text, letterSpacing: "-0.01em" }}>
                Per<span style={{ color: T.primary }}>Diem</span><span style={{ fontWeight: 400, color: T.textTertiary, fontSize: "13px" }}>.fyi</span>
            </span>
        </div>
    );
}
function GovBadge({ text }: { text: string }) {
    return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: "4px", fontFamily: f.sans, fontSize: "10px", fontWeight: 600, color: T.accent, background: T.accentMuted, border: `1px solid ${T.accentBorder}`, padding: "2px 8px", borderRadius: "4px", letterSpacing: "0.04em" }}>
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={T.accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
            {text}
        </span>
    );
}
function MicroLabel({ children }: { children: React.ReactNode }) {
    return <span style={{ fontFamily: f.sans, fontSize: "10px", fontWeight: 700, color: T.textTertiary, letterSpacing: "0.08em", textTransform: "uppercase" as const }}>{children}</span>;
}
function Card({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) {
    return <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: "12px", padding: "20px", ...style }}>{children}</div>;
}
function Pill({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
    return (
        <button onClick={onClick} style={{ border: `1px solid ${active ? T.borderFocus : T.border}`, background: active ? T.primaryMuted : T.surface, color: active ? T.primary : T.textSecondary, borderRadius: "999px", padding: "7px 12px", fontFamily: f.sans, fontSize: "12px", fontWeight: 700, cursor: "pointer", transition: "all 0.15s ease" }}>
            {label}
        </button>
    );
}
function ProgressBar({ value, max, color }: { value: number; max: number; color: string }) {
    ...
    return (
        <div style={{ width: "100%", height: "6px", background: T.surfaceRaised, borderRadius: "99px", overflow: "hidden" }}>
...
export default function ConstructionCalculator() {
    ...
    const fetchPreview = async (z: string) => {
        ...
        try {
            const res = await fetch("/api/v1/lookup-stipend", {
                ...
                body: JSON.stringify({ zip: z, mode: "healthcare", gross_weekly: 2000, hours: 40, ingest: false, insurance_plan: "none" }),
            });
            if (!res.ok) throw new Error("Lookup failed");
            const json = await res.json();
            if (json.success) { setGsaPreview(json.data); setStep(1); }
        } catch (e: unknown) {
    ...
    const handleDecode = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch("/api/v1/lookup-stipend", {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    zip, mode: "construction", trade, schedule,
                    hourly_rate: rate, daily_per_diem: pd || 0, per_diem_days: pdDays,
            ...
            const json = await res.json();
            if (json.success) { setResult(json.data as ConstructionResult); setStep(2); }
        } catch (e: unknown) {
            setError(e instanceof Error ? e.message : "Failed");
        } finally { setLoading(false); }
    }, [zip, trade, schedule, hourlyRate, dailyPd, pdDays, housingModel, agency, plan, insOverride]);

    const handleReset = useCallback(() => {
        setStep(0); setZip(""); setTrade("electrician"); setSchedule("5x10");
        setHourlyRate(""); setDailyPd(""); setPdDays(7); setHousingModel("self");
        setAgency(""); setPlan("none"); setInsOverride("");
        setGsaPreview(null); setResult(null); setError(null);
        if (typeof window !== "undefined") window.history.replaceState({}, "", window.location.pathname + "?mode=construction");
    }, []);
    ...
    const handleShare = useCallback(async () => {
        ...
        if (navigator.share) await navigator.share({ title: "PerDiem.fyi", text, url });
    ...
    if (error) return (
        <div style={{ background: T.bg, minHeight: "100vh", color: T.text, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
            <Card style={{ maxWidth: "360px", textAlign: "center" }}>
                ...
                <p style={{ fontFamily: f.sans, fontSize: "13px", color: T.textSecondary, margin: "0 0 16px", lineHeight: 1.5 }}>{error}</p>
                <button onClick={handleReset} style={{ fontFamily: f.sans, fontSize: "14px", fontWeight: 800, padding: "10px 32px", borderRadius: "8px", border: "none", background: T.primary, color: "#fff", cursor: "pointer" }}>Try again</button>
            </Card>
        </div>
    );

    if (loading) return (
        <div style={{ background: T.bg, minHeight: "100vh", color: T.text, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "24px" }}>
            <Logo />
        ...
        <div style={{ background: T.bg, minHeight: "100vh", color: T.text, display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 20px" }}>
            <div style={{ width: "100%", maxWidth: "360px", display: "flex", flexDirection: "column", gap: "18px", alignItems: "center" }}>
                ...
                <div style={{ textAlign: "center" }}>
                    <div style={{ fontFamily: f.sans, fontSize: "24px", fontWeight: 900, letterSpacing: "-0.02em" }}>Is this per diem good?</div>
                    <div style={{ fontFamily: f.sans, fontSize: "14px", color: T.textSecondary, marginTop: "6px", lineHeight: 1.5 }}>Construction trades per diem decoded against GSA rates.</div>
                </div>
                <div style={{ width: "100%" }}>
                    <label style={{ fontFamily: f.sans, fontSize: "12px", fontWeight: 800, color: T.textSecondary, display: "block", marginBottom: "6px" }}>Jobsite ZIP</label>
                    <input value={zip} onChange={(e) => setZip(e.target.value.replace(/\\D/g, "").slice(0, 5))} onKeyDown={(e) => e.key === "Enter" && handleZipSubmit()} inputMode="numeric" placeholder="77001" autoFocus
                        style={{ width: "100%", boxSizing: "border-box", fontFamily: f.mono, fontSize: "24px", fontWeight: 900, padding: "14px 16px", borderRadius: "10px", border: `2px solid ${zip.length === 5 ? T.borderFocus : T.border}`, background: T.surface, color: T.text, outline: "none", textAlign: "center", letterSpacing: "0.1em" }} />
                </div>
                <button onClick={handleZipSubmit} disabled={zip.length !== 5}
            ...
            <div style={{ background: T.bg, minHeight: "100vh", color: T.text, padding: "28px 16px", display: "flex", justifyContent: "center" }}>
                <div style={{ width: "100%", maxWidth: "420px", display: "flex", flexDirection: "column", gap: "14px" }}>
                    ...
                    <Card>
                        ...
                        <div style={{ marginTop: "8px", fontFamily: f.mono, fontSize: "34px", fontWeight: 900 }}>
                            ${gs.weekly_max.toLocaleString()}<span style={{ fontSize: "16px", fontWeight: 400, color: T.textTertiary }}>/wk</span>
                        </div>
                        <div style={{ fontFamily: f.sans, fontSize: "13px", color: T.textSecondary, marginTop: "4px" }}>
                    ...
                    <Card>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "10px" }}>
                            {TRADES.map((t) => <Pill key={t.value} label={t.label} active={trade === t.value} onClick={() => setTrade(t.value)} />)}
                        </div>
                    </Card>
                    ...
                    <Card>
                        ...
                        <div style={{ display: "flex", gap: "8px", marginTop: "10px", flexWrap: "wrap" }}>
                            {SCHEDULES.map((s) => <Pill key={s.value} label={s.label} active={schedule === s.value} onClick={() => setSchedule(s.value)} />)}
                        </div>
                        <div style={{ fontFamily: f.sans, fontSize: "11px", color: T.textTertiary, marginTop: "8px" }}>
                            {SCHEDULES.find((s) => s.value === schedule)?.hours ?? 50} hrs/wk · OT at 1.5× after 40hr
                        </div>
                    </Card>
                        ...
                        <div style={{ display: "flex", gap: "12px" }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ position: "relative", marginTop: "8px" }}>
                                    <span style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", fontFamily: f.mono, fontSize: "18px", fontWeight: 900, color: T.textTertiary }}>$</span>
                                    <input value={hourlyRate} onChange={(e) => setHourlyRate(e.target.value.replace(/[^\\d.]/g, ""))} onKeyDown={(e) => e.key === "Enter" && handleDecode()} inputMode="decimal" placeholder={String(tradeObj?.defaultRate ?? 35)} autoFocus
                                        style={{ width: "100%", boxSizing: "border-box", fontFamily: f.mono, fontSize: "18px", fontWeight: 900, padding: "12px 12px 12px 30px", borderRadius: "10px", border: `2px solid ${hourlyRate ? T.borderFocus : T.border}`, background: T.surface, color: T.text, outline: "none" }} />
                                </div>
                            </div>
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ position: "relative", marginTop: "8px" }}>
                                    <span style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", fontFamily: f.mono, fontSize: "18px", fontWeight: 900, color: T.textTertiary }}>$</span>
                                    <input value={dailyPd} onChange={(e) => setDailyPd(e.target.value.replace(/[^\\d.]/g, ""))} onKeyDown={(e) => e.key === "Enter" && handleDecode()} inputMode="decimal" placeholder="120"
                                        style={{ width: "100%", boxSizing: "border-box", fontFamily: f.mono, fontSize: "18px", fontWeight: 900, padding: "12px 12px 12px 30px", borderRadius: "10px", border: `2px solid ${dailyPd ? T.borderFocus : T.border}`, background: T.surface, color: T.text, outline: "none" }} />
                                </div>
                            </div>
                        </div>
                        <div style={{ display: "flex", gap: "12px", marginTop: "12px" }}>
                            <div>
                                ...
                                <div style={{ display: "flex", gap: "6px", marginTop: "6px" }}>
                                    {([5, 6, 7] as const).map((d) => <Pill key={d} label={`${d}`} active={pdDays === d} onClick={() => setPdDays(d)} />)}
                                </div>
                            </div>
                            <div>
                                <MicroLabel>Housing</MicroLabel>
                                <div style={{ display: "flex", gap: "6px", marginTop: "6px" }}>
                                    <Pill label="I find my own" active={housingModel === "self"} onClick={() => setHousingModel("self")} />
                                    <Pill label="Company provides" active={housingModel === "company"} onClick={() => setHousingModel("company")} />
                                </div>
                            </div>
                        </div>
                    </Card>
                    ...
                    <Card>
                        ...
                        <input value={agency} onChange={(e) => setAgency(e.target.value)} placeholder="Type or tap below"
                            style={{ width: "100%", marginTop: "8px", boxSizing: "border-box", fontFamily: f.sans, fontSize: "15px", fontWeight: 700, padding: "12px", borderRadius: "10px", border: `2px solid ${agency.trim() ? T.borderFocus : T.border}`, background: T.surface, color: T.text, outline: "none" }} />
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "10px" }}>
                            {AGENCY_CHIPS.map((x) => (
                                <button key={x} onClick={() => setAgency(x)} style={{ border: `1px solid ${T.border}`, background: agency === x ? T.primaryMuted : T.surface, color: agency === x ? T.primary : T.textSecondary, borderRadius: "999px", padding: "6px 10px", fontFamily: f.sans, fontSize: "12px", fontWeight: 700, cursor: "pointer" }}>{x}</button>
                            ))}
                        </div>
                    </Card>
                    ...
                    <Card>
                        ...
                        <div style={{ display: "flex", gap: "8px", marginTop: "10px", flexWrap: "wrap" }}>
                            {PLAN_OPTIONS.map((o) => <Pill key={o.value} label={o.label} active={plan === o.value} onClick={() => setPlan(o.value)} />)}
                        </div>
                        {plan === "union" && (
                            <div style={{ fontFamily: f.sans, fontSize: "12px", color: T.primary, marginTop: "8px", padding: "8px", background: T.primaryMuted, borderRadius: "6px" }}>
                                Union trust fund — $0 weekly premium deduction. Funded by employer contributions per hour worked.
                            </div>
                        )}
                        {plan !== "none" && plan !== "union" && (
                            <div style={{ marginTop: "10px" }}>
                                <div style={{ fontFamily: f.sans, fontSize: "12px", color: T.textSecondary, marginBottom: "6px" }}>Weekly premium override</div>
                                <div style={{ position: "relative" }}>
                                    <span style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", fontFamily: f.mono, fontSize: "16px", color: T.textTertiary }}>$</span>
                                    <input value={insOverride} onChange={(e) => setInsOverride(e.target.value.replace(/[^\\d.]/g, ""))} inputMode="decimal" placeholder="0"
                                        style={{ width: "100%", boxSizing: "border-box", fontFamily: f.mono, fontSize: "16px", fontWeight: 900, padding: "10px 10px 10px 28px", borderRadius: "8px", border: `1px solid ${T.border}`, background: T.surface, color: T.text, outline: "none" }} />
                                </div>
                            </div>
                        )}
                    </Card>
                    ...
                    {/* DECODE */}
                    <button onClick={handleDecode} disabled={!canDecode}
                        style={{ width: "100%", fontFamily: f.sans, fontSize: "15px", fontWeight: 900, padding: "14px", borderRadius: "10px", border: "none", background: canDecode ? T.primary : T.surfaceRaised, color: canDecode ? "#fff" : T.textTertiary, cursor: canDecode ? "pointer" : "default" }}>
                        Decode my offer →
                    </button>
                </div>
            </div>
    ...
    if (step === 2 && result) {
        ...
        const tradeLabel = TRADES.find((t) => t.value === c.trade)?.label ?? c.trade;
            ...
            <div style={{ background: T.bg, minHeight: "100vh", color: T.text, padding: "18px 16px 80px", display: "flex", justifyContent: "center" }}>
                <div style={{ width: "100%", maxWidth: "460px" }}>
                    ...
                    <div style={{ position: "sticky", top: 0, zIndex: 10, background: T.bg, padding: "12px 0 10px", borderBottom: `1px solid ${T.borderSubtle}`, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                        <button onClick={handleReset} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}><Logo /></button>
                        <div style={{ textAlign: "right" }}>
                            <div style={{ fontFamily: f.sans, fontSize: "10px", fontWeight: 900, color: T.textTertiary, letterSpacing: "0.08em", textTransform: "uppercase" }}> Net after insurance</div>
                            <div style={{ fontFamily: f.mono, fontSize: "18px", fontWeight: 900, color: T.moneyPositive }}>$${r.derived.net_after_insurance_weekly.toLocaleString()}/wk</div>
                        </div>
                    </div>
                    ...
                    <Card>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                                <span style={{ fontFamily: f.sans, fontSize: "13px", color: T.textSecondary }}>Weekly Total</span>
                                <span style={{ fontFamily: f.mono, fontSize: "16px", fontWeight: 900 }}>$${c.weekly_gross_total.toLocaleString()}</span>
                            </div>
                            ...
                            <div>
                                ...
                                <ProgressBar value={c.straight_pay} max={c.weekly_gross_total} color={T.textTertiary} />
                            </div>

                            {c.ot_hours > 0 && (
                                <div>
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                        <span style={{ fontFamily: f.sans, fontSize: "12px", color: T.accent }}>
                                            OT ({c.ot_hours}hr × $${Math.round(c.hourly_rate * 1.5 * 100) / 100})
                                        </span>
                                        <span style={{ fontFamily: f.mono, fontSize: "14px", fontWeight: 700, color: T.accent }}>$${c.ot_pay.toLocaleString()}</span>
                                    </div>
                                    <ProgressBar value={c.ot_pay} max={c.weekly_gross_total} color={T.accent} />
                                </div>
                            )}
                            ...
                            <div>
                                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                    <span style={{ fontFamily: f.sans, fontSize: "12px", color: T.primary, fontWeight: 700 }}>
                                        Per Diem ($${c.daily_per_diem}/day × {c.per_diem_days}d) — tax-free
                                    </span>
                                    <span style={{ fontFamily: f.mono, fontSize: "14px", fontWeight: 700, color: T.primary }}>$${c.weekly_per_diem.toLocaleString()}</span>
                                </div>
                            ...
                            {insMid > 0 && (
                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                    ...
                                    <span style={{ fontFamily: f.mono, fontSize: "14px", fontWeight: 900, color: T.moneyNegative }}>−$${Math.round(insMid)}/wk</span>
                                </div>
                            )}
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 12px", background: T.moneyPositiveBg, borderRadius: "8px", border: `1px solid ${T.moneyPositive}20` }}>
                                ...
                                <span style={{ fontFamily: f.mono, fontSize: "16px", fontWeight: 900, color: T.moneyPositive }}>$${r.derived.net_after_insurance_weekly.toLocaleString()}/wk</span>
                            </div>
                        </div>
                    ...
                    {c.housing_model === "self" && (
                        <Card style={{ marginTop: "12px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
                                ...
                                <span style={{ fontFamily: f.sans, fontSize: "9px", color: T.textTertiary, background: T.surfaceRaised, padding: "2px 6px", borderRadius: "3px" }}>{r.metadata.hud_rent_source}</span>
                            </div>
                    ...
                    <Card style={{ marginTop: "12px", borderColor: `${T.accent}40` }}>
                        <div style={{ display: "flex", gap: "8px", alignItems: "flex-start" }}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={T.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginTop: "2px", flexShrink: 0 }}>
                                <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" />
                            </svg>
                            <div style={{ fontFamily: f.sans, fontSize: "12px", color: T.textSecondary, lineHeight: 1.5 }}>
                                <strong style={{ color: T.accent }}>IRS 1-Year Rule:</strong> Per diem is tax-free only if your assignment at this location is expected to last under 12 months. Longer projects → per diem becomes taxable wages. Consult a tax professional.
                            </div>
                        </div>
                    </Card>
                    ...
                    <footer style={{ fontFamily: f.sans, fontSize: "10px", color: T.textTertiary, lineHeight: 1.6, padding: "16px 0", borderTop: `1px solid ${T.borderSubtle}`, marginTop: "12px" }}>
                        ...
                        OT calculated at 1.5× after 40hr/wk per FLSA. Not tax advice. Not sponsored by any agency.
                    </footer>
                </div>
            </div>
        );
